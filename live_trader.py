...
from broker import (
    ...
    AuthentifizierungsFehler, NichtUnterstuetzt,
)
...
def _etoro_exit_attribution(*, broker, fill, rec_before, meta: dict,
    ...
    """Ordnet brokerseitige eToro-SL/TP-Fills einer Bot-Position zu.

    eToro liefert historische Positionsschliessungen mit einer synthetischen
    Close-ID. Diese ID kann nicht in der lokalen Order-Registry stehen. Eine
    Zuordnung ist deshalb nur erlaubt, wenn eine AUTO-Position des Bots
    existiert und entweder der Brokergrund oder der Ausfuehrungspreis den
    gespeicherten Schutzwert belastbar bestaetigt.
    """
    result = dict(meta or {})
    if str(getattr(broker, "name", "") or "").lower() != "etoro" or rec_before is None:
        return sell_bot_owned, result
    if result.get("label") or result.get("reason"):
        return sell_bot_owned, result

    mode = str(getattr(rec_before, "management_mode", "") or "").upper()
    source = str(getattr(rec_before, "source", "") or "").upper()
    eigene_auto_position = mode == "AUTO" and source in {"BOT", "USER_MANAGED"}
    raw_reason = str(getattr(fill, "execution_reason", "") or "").strip()
    norm = raw_reason.upper().replace("_", " ").replace("-", " ")
    label = ""
    reason = ""
    if eigene_auto_position and ("STOP" in norm or norm in {"SL", "S L"}):
        label, reason = "STOP-LOSS", "Brokerseitiger Stop-Loss ausgefuehrt"
    elif eigene_auto_position and ("TAKE PROFIT" in norm or norm in {"TP", "T P"}):
    ...
    if eigene_auto_position and not label and price > 0:
    ...
    result.update({
        "label": "MANUELLER BROKER-VERKAUF",
    ...
    for raw_fill in fills:
        ...
        try:
            ...
            contract = (
                ...
                or _ErsatzKontrakt(symbol, fill.currency, fill.broker_id)
            )
            ...
            if side.startswith("BUY"):
                ...
                fill_owner = classify_fill_owner(meta, ownership_registry, fill.order_id)
                ...
                rec = manager.register_buy(
                    ...
                    management_mode="AUTO" if bot_owned else "OBSERVE",
                )
                ...
                if bot_owned:
                    risk.register_estimated_cost(one_way_cost)
                    try:
                    ...
                    try:
                        ...
                        trade_ledger.trade_open(
                            ...
                            gebuehr=one_way_cost, paper=bool(broker.ist_paper()),
                        ...
                        logger.debug("Trade-Ledger Einstieg eToro nicht geschrieben: %s", exc)
                else:
                    try:
                ...
                sell_owner = classify_fill_owner(meta, ownership_registry, fill.order_id)
                ...
                if pnl_known:
                    ...
                    if sell_bot_owned:
                        risk.register_realized_pnl(
                            pnl, equity, gross_pnl=gross_trade_pnl,
                            trade_id=f"{getattr(broker, 'name', '?')}:{fill.order_id}",
                        )
                else:
                    ...
                    pnl_pct = None
                    if sell_bot_owned:
                        risk.register_unknown_pnl_trade(trade_id=f"{getattr(broker, 'name', '?')}:{fill.order_id}")
                try:
                    import trade_ledger
                    trade_ledger.trade_close(
                        ...
                        gebuehr=one_way_cost, netto_pnl=pnl,
                    ...
                    logger.debug("Trade-Ledger Ausstieg eToro nicht geschrieben: %s", exc)
    ...
    try:
        ...
        for symbol, ids in evidence.items():
            count += int(trade_ledger.confirm_open_trade(
                ...
                broker_position_id=",".join(sorted(str(x) for x in ids))) or 0)
        return count
    except Exception as exc:
...
def run():
    ...
    def ensure_connection(force_probe: bool = False):
        ...
        # eToro-Healthcheck prueft aktiv die API-Erreichbarkeit.
        try:
    ...
    def journal_ablehnung(inst, grund_code: str, grund_text: str,
        ...
        try:
            ...
            logger.debug("Ablehnung konnte nicht journalisiert werden: %s", exc)
    ...
    try:
        ...
        while True:
            try:
                ...
                # Bereits bestehende Ausfuehrungen beim Start nicht als neue Trades melden.
        ...
        def _health_text() -> str:
            ...
            try:
                ...
                lines.append(("🟢" if not bad else "🟡") + f" Newsquellen: {good} OK, {disabled} aus/unbekannt" + (f", gestört: {', '.join(bad[:3])}" if bad else ""))
            except Exception as exc:
        ...
        if str(getattr(broker, "name", "")).lower() == "etoro":
            try:
                ...
                stock_bereitschaft.melde(
                    "keine_unklare_order",
                    not bool(etoro_reconciliation.status().get("active")),
                    "keine ungeklärte eToro-Order" if not etoro_reconciliation.status().get("active")
                    else "eToro-Reconciliation noch offen")
                stock_bereitschaft.melde("reconciliation", True,
                                          "Depot, Orders und Historie abgeglichen")
            except Exception as exc:
                logger.warning("eToro-Startabgleich bleibt offen: %s", exc)
                stock_bereitschaft.melde("reconciliation", False, str(exc)[:160])
        ...
        def resync_after_reconnect():
            ...
            for attempt in range(1, attempts + 1):
                try:
                    ...
                    risk.update_equity_guard(equity_now)
                    ...
                    for _key, (contract, qty, _avg, inst) in list(current_positions.items()):
                        ...
                        if result.get("checked") and (result.get("stop_ok") is False or "abweichend" in detail.lower()):
        ...
        while True:
            ...
            risk.update_equity_guard(equity)
            stock_bereitschaft.melde("broker_verbunden", bool(broker.is_connected()))
            ...
            try:
                ...
                stock_bereitschaft.melde(
                    "keine_unklare_order",
                    not bool(etoro_reconciliation.status().get("active")),
                    "keine ungeklärte eToro-Order")
            except Exception as exc:
                stock_bereitschaft.melde("keine_unklare_order", False, str(exc)[:160])
            ...
            try:
                ...
                current_positions = manager.sync_with_broker(broker, universe)
                _confirm_ledger_from_current_positions(broker)
                risk.open_positions = sum(1 for _, (_, qty, _, _) in current_positions.items() if qty != 0)
                risk.save()
            ...
            for con_id, (contract, qty, avg_cost, inst) in list(current_positions.items()):
                ...
                if mode_owner == "PENDING_TAKEOVER":
                    try:
                        ...
                        if protected:
                            manager.confirm_takeover(inst.name, detail)
                            mode_owner="AUTO"
                            notify("BOT-VERWALTUNG UEBERNOMMEN", f"{inst.name}: {detail}")
                        elif checked:
                            manager.reject_takeover(inst.name, detail)
                            mode_owner="OBSERVE"
                            notify("UEBERNAHME NICHT AKTIVIERT", f"{inst.name}: {detail}. Position bleibt nur beobachtet.")
                        else:
                            manager.reject_takeover(inst.name, detail or "Broker kann Schutz fuer bestehende Position nicht sicher bestaetigen")
                            mode_owner="OBSERVE"
                        ...
                        if _connection_failure(broker, exc):
                            raise
                        manager.reject_takeover(inst.name, f"Schutzpruefung fehlgeschlagen: {exc}")
                        mode_owner="OBSERVE"
                        logger.warning("Uebernahmepruefung %s fehlgeschlagen: %s",inst.name,exc)
                if mode_owner != "AUTO":
                    print(f"POS {inst.name:8s} OBSERVE | Herkunft={getattr(rec_owner,'source','?')} | keine automatischen Exits")
                    continue
                try:
                    df = broker.historie(
                    ...
                    if df is not None and len(df) >= 30:
                        stock_bereitschaft.melde(
                            "kursdaten", True,
                            f"{inst.name}: {len(df)} abgeschlossene/auswertbare Kerzen")
                    ...
                    # --- Time Stop: Positionen ohne Fortschritt nicht endlos halten ---
                    if darf_verkaufen and getattr(config, "TIME_STOP_ENABLED", True) and qty > 0:
                        try:
                            rec_ts = manager.get(contract)
                            if rec_ts and rec_ts.entry_time and avg_cost > 0 and signal.price > 0:
                                start_dt = datetime.fromisoformat(rec_ts.entry_time)
                                alter_h = max(0.0, (datetime.now(start_dt.tzinfo) - start_dt).total_seconds()/3600.0)
                                ret = signal.price / avg_cost - 1.0
                                if alter_h >= float(getattr(config, "TIME_STOP_HOURS", 72)) and ret < float(getattr(config, "TIME_STOP_MIN_RETURN_PCT", 0.005)):
                                    print(f"  !! TIME-STOP {inst.name}: {alter_h:.0f}h gehalten, Ergebnis {ret*100:+.2f}%")
                                    ergebnis_t = close_position(broker, inst, qty, asset_type=inst.asset_type, reference_price=signal.price)
                                    for oid in ergebnis_t.order_ids:
                                        order_meta[str(oid)] = {"reason": f"Time-Stop nach {alter_h:.0f}h ({ret*100:+.2f}%)", "label": "TIME-STOP"}
                                    continue
                        except Exception as exc:
                            if _connection_failure(broker, exc):
                                raise
                            logger.warning("Time-Stop %s: %s", inst.name, exc)
                    lage_pos = None
                    ...
                    if (darf_verkaufen and qty > 0
                        ...
                        try:
                            ...
                            if lage_pos.verkauf_empfohlen:
                                ...
                                if ergebnis_n and getattr(ergebnis_n, "order_id", None):
                                    order_meta[str(ergebnis_n.order_id)] = {
                                        "reason": f"Nachrichtenlage: {lage_pos.kurzfassung()}",
                    ...
                    if signal.action == "SELL" and qty > 0 and not darf_verkaufen:
                        ...
                        ergebnis = close_position(
                            broker, inst, qty,
                            asset_type=inst.asset_type, reference_price=signal.price,
                        )
                        for oid in ergebnis.order_ids:
                            order_meta[str(oid)] = {
            ...
            for inst in block:
                ...
                try:
                    ...
                    if time.time() - last < config.INSTRUMENT_COOLDOWN_MINUTES * 60:
                        continue
                    # Brokerbestand nochmals abfragen. Kein BUY auf bestehende Position.
                    ...
                    if df is not None and len(df) >= 30:
                        stock_bereitschaft.melde(
                            "kursdaten", True,
                            f"{inst.name}: {len(df)} auswertbare Kerzen im Scanner")
                    ...
                    if getattr(config, "NEWS_FILTER_ENABLED", True) and inst.asset_type != "forex":
                        try:
                            ...
                            grenze = (getattr(config, "NEWS_BLOCK_SCORE_UNDERDOG", 3)
                                      if konservativer_kleinwert
                                      else getattr(config, "NEWS_BLOCK_SCORE", 5))
                    ...
                    stop, take = calculate_stop_take(signal.price, "BUY", signal.atr, inst.asset_type)
                    ...
                    max_pct = config.CRYPTO_MAX_POSITION_PCT if inst.asset_type == "crypto" else config.MAX_POSITION_PCT
                    if ist_broad and inst.asset_type=="stock":
                    ...
                    if (entry_session is not None and getattr(entry_session,"session","") == "EXTENDED" and
                            broker_name == "etoro"):
                        try:
                            broker.dynamic_cost_quote(inst, qty, signal.price, action="open")
                        except Exception as exc:
                    ...
                    try:
                        kosten = estimate_roundtrip_with_broker(
                            broker, qty, signal.price, inst.asset_type, inst.currency,
                    ...
                    move_est = estimate_plausible_move(
                        signal.price, take, atr_value=signal.atr, event=event,
                    ...
                    risk_amount = abs(signal.price - stop) * qty
                    position_value = signal.price * qty
                    print(
                    ...
                    decision_id = record_decision(
                        ...
                        status="APPROVED", broker=getattr(broker,"name","?"),
                        paper=bool(getattr(config,"PAPER_TRADING",True)), symbol=inst.name,
                    ...
                    execution = submit_protected_buy(
                        broker, inst, qty, signal.price, stop, take, decision_id=decision_id
                    )
                    ...
                    order_meta.pop(f"PENDING_BUY:{canonical_key(inst.name, inst.asset_type)}", None)
                    ownership_registry.clear_pending(inst.name, inst.asset_type)
                    last_data_error[inst.name] = time.time()
